	...
	public class ForestGuardian : ModNPC
	{
		...
		private void Slam()
		{
			...
			if (npc.collideY && npc.position.Y == npc.oldPosition.Y)
			{
				...
				Main.player[npc.target].GetModPlayer<Common.Players.TrelamiumPlayer>().shakeEffects = Math.Abs(npc.velocity.Y * 3.85f);
			}
		}
		...
		private void HandleScreenText(SpriteBatch spriteBatch)
		{
			...
			DrawHelpers.DrawText(spriteBatch, position, "- Forest Guardian -", color);
		}
	}
}
