...
namespace TrelamiumTwo.Content.Projectiles.Misc
{
	public sealed class FloaterMelee : ModProjectile
	{
		...
		public override bool PreAI()
		{
			...
			TrelamiumPlayer trelamiumPlayer = owner.GetModPlayer<TrelamiumPlayer>();

			CheckAliveState(trelamiumPlayer);
		...
		private void CheckAliveState(TrelamiumPlayer trelamiumPlayer)
		{
			if (!trelamiumPlayer.magicGuantlet)
			{
				projectile.Kill();
				return;
			}
		...
		private void AttackNPC(NPC npc)
		{
			...
			else
			{
				...
				Dust.NewDustPerfect(projectile.Center + (projectile.rotation - MathHelper.PiOver2).ToRotationVector2() * projectile.width,
					245, projectile.velocity * 0.2f, 100);
