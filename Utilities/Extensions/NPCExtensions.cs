...
namespace TrelamiumTwo.Utilities.Extensions
{
	internal static class NPCExtensions
	{
		...
		public static void DrawNPCTrailCenteredWithTexture(this NPC npc, Texture2D texture, SpriteBatch spriteBatch, Color color, float initialOpacity = 0.8f, float opacityDegrade = 0.2f, int stepSize = 1)
		{
			...
			for (int i = 0; i < ProjectileID.Sets.TrailCacheLength[npc.type]; i += stepSize)
			{
				float opacity = initialOpacity - opacityDegrade * i;

				Vector2 position = npc.oldPos[i] + npc.Hitbox.Size() / 2f - Main.screenPosition + new Vector2(0f, npc.gfxOffY);
